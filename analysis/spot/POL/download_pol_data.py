...
# Exchange-specific POL/USD symbol mapping
SYMBOL_MAP = {
    "binance": ["POLUSDT"],
    "coinbase": ["POL-USD"],  # may fail if delisted/not listed
    "gemini": ["POLUSD"],      # placeholder; will fail if not listed
}

def download_spot_trades():
    """
    Downloads POL spot trading data across binance/coinbase/gemini for 2025-10-10.
    """
    ...
    print("Starting POL spot trade data downloads...")
    ...
    for exchange in EXCHANGES:
        exchange_symbols = SYMBOL_MAP.get(exchange, ["POLUSDT"])
        success = 0
        ...
        if success == 0:
            print(f"⚠ No successful downloads for {exchange}. Check symbol mapping or availability.")
    ...
    if subdirs:
        ...
        print("\n⚠ Warning: No files found. Check API key access and date availability.")


def compare_prices():
    """
    Compare minimum prices across exchanges for POL.
    """
    script_dir = os.path.dirname(os.path.abspath(__file__))
    download_dir = script_dir

    print("\n--- Price Comparison Analysis (POL) ---")
